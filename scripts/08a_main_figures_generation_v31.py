...
"""
=============================================================================
COMPLETE FIGURE AND TABLE GENERATION SCRIPT
...
OUTPUT_DIR = '/home/claude/figures_v31/'
...
def load_data(filepath=None):
    ...
    if filepath is None:
        ...
        possible_paths = [
            '/mnt/project/07b_DE_results_1054_strains_v31.xlsx',
            os.path.join(os.path.dirname(os.path.abspath(__file__)), '07b_DE_results_1054_strains_v31.xlsx'),
            os.path.join(DATA_DIR, '07b_DE_results_1054_strains_v31.xlsx'),
        ]
...
def generate_figure1(df, output_dir=OUTPUT_DIR):
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}05a_Figure1_model_parameters_v31.pdf', bbox_inches='tight')
    plt.savefig(f'{output_dir}05a_Figure1_model_parameters_v31.png', bbox_inches='tight')
    plt.close()
...
def generate_figure2(df, stats_dict, output_dir=OUTPUT_DIR):
    ...
    ax_A_top.set_title('A  Dual-population $E_h$ distribution',
    ...
    # Save figure
    plt.savefig(f'{output_dir}05b_Figure2_unified_strategies_v31.pdf', bbox_inches='tight', dpi=300)
    plt.savefig(f'{output_dir}05b_Figure2_unified_strategies_v31.png', bbox_inches='tight', dpi=300)
    plt.close()
...
def generate_figure3(df, output_dir=OUTPUT_DIR):
    ...
    plt.savefig(f'{output_dir}05c_Figure3_ab_initio_validation_v31.pdf', bbox_inches='tight')
    plt.savefig(f'{output_dir}05c_Figure3_ab_initio_validation_v31.png', bbox_inches='tight')
    plt.close()
...
def generate_figure4(df, output_dir=OUTPUT_DIR):
    ...
    # "Zero-parameter extrapolation" annotation with arrow pointing to Yvon-Durocher diamond
    ax.annotate('Zero-parameter\nextrapolation', xy=(yd_lo - 0.005, y_eco),
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}05d_Figure4_warming_feedback_v31.pdf', bbox_inches='tight')
    plt.savefig(f'{output_dir}05d_Figure4_warming_feedback_v31.png', bbox_inches='tight')
    plt.close()
...
def generate_tables(df, output_dir=OUTPUT_DIR):
    ...
    table1.to_csv(f'{output_dir}04_Table1_parameters_by_domain_v31.csv', index=False, encoding='utf-8')
    ...
    table2.to_csv(f'{output_dir}04_Table2_Q10_by_trophic_mode_v31.csv', index=False, encoding='utf-8')
    ...
    table3.to_csv(f'{output_dir}04_Table3_Er_by_O2_metabolism_v31.csv', 
...
def generate_aposteriori_summary(df):
    ...
    results_df.to_csv(f'{OUTPUT_DIR}04a_Table_strategy_by_group_v31.csv', index=False)
    print(f"\n  [OK] Saved: 04a_Table_strategy_by_group_v31.csv")
