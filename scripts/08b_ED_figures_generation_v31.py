...
def load_data():
    ...
    df = pd.read_excel(f'{DATA_DIR}07b_DE_results_1054_strains_v31.xlsx')
    ...
    metgroup_map = {
        ...
        'Anoxygenic_phototroph': 'Anoxy. phototrophs',
        'Methanogen': 'Methanogens',
        'Chemolithoautotroph': 'Chemoautotrophs',
        'Other': 'Other',
    }
...
def plot_ed_fig1(df, growth_data, output_dir=OUTPUT_DIR):
    """
    Extended Data Figure 1. Model fit quality.
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}06a_ED_Fig1_model_fit_quality_v31.png', bbox_inches='tight')
    plt.close()
...
def plot_ed_fig2(df, output_dir=OUTPUT_DIR):
    """
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}06b_ED_Fig2_guild_validation_v31.png', bbox_inches='tight')
    plt.close()
...
def plot_ed_fig3(df, output_dir=OUTPUT_DIR):
    """
    Extended Data Figure 3. Thermal niche distribution.
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}06c_ED_Fig3_thermal_niche_v31.png', bbox_inches='tight')
    plt.close()
...
def plot_ed_fig4(df, output_dir=OUTPUT_DIR):
    """
    Extended Data Figure 4. Parameter distributions by domain.
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}06d_ED_Fig4_parameter_distributions_v31.png', bbox_inches='tight')
    plt.close()
...
def plot_ed_fig5(df, output_dir=OUTPUT_DIR):
    """
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}06e_ED_Fig5_Q10_mesophiles_v31.png', bbox_inches='tight')
    plt.close()
...
def plot_ed_fig6(df, output_dir=OUTPUT_DIR):
    """
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}06f_ED_Fig6_equilibrium_proximity_v31.png', bbox_inches='tight')
    plt.close()
...
def plot_ed_fig7(df_de, df_ctm, output_dir=OUTPUT_DIR):
    """
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}06g_ED_Fig7_model_comparison_v31.png', bbox_inches='tight')
    plt.close()
...
def plot_ed_fig8(df_de, df_mcmc, output_dir=OUTPUT_DIR):
    """
    Extended Data Figure 8. DE vs MCMC parameter estimation validation.
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}06h_ED_Fig8_DE_vs_MCMC_v31.png', bbox_inches='tight')
    plt.close()
...
# =============================================================================
def generate_ed_fig9(df, output_dir=OUTPUT_DIR):
    ...
    outpath = os.path.join(output_dir, '06i_ED_Fig9_multivariate_v31.png')
...
def generate_ed_fig10(df, output_dir=OUTPUT_DIR):
    ...
    plt.tight_layout()
    plt.savefig(f'{output_dir}06j_ED_Fig10_oxygen_metabolism_v31.pdf', bbox_inches='tight')
    plt.savefig(f'{output_dir}06j_ED_Fig10_oxygen_metabolism_v31.png', bbox_inches='tight')
    plt.close()
...
def main():
    ...
    print("=" * 70)
    print("GENERATING ALL EXTENDED DATA FIGURES 1-10 (v31)")
    print("=" * 70)
    ...
    # Load data
    df = load_data()
    growth_data = pd.read_excel(f'{DATA_DIR}07a_growth_data_1054_strains_v31.xlsx')
    df_ctm = pd.read_excel(f'{DATA_DIR}07d_CTM_comparison_results_v31.xlsx')
    df_mcmc = pd.read_excel(f'{DATA_DIR}07c_MCMC_results_1054_strains_v31.xlsx')
